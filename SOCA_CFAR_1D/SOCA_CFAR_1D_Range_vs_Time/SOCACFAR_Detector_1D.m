function CA = SOCACFAR_Detector_1D(PFA, referenceCells, guardCells, dataSize, signal, aCA)
    ...
    for CUT = 1: length(signal)
        ...
        elseif CUT > referenceCells/2 && CUT < length(signal) - referenceCells/2
            LaggingWindow = sum(DataAfterPowerLawDetector( (CUT-referenceCells/2):(CUT-guardCells/2))); 
            LeadingWindow = sum(DataAfterPowerLawDetector( (CUT+guardCells/2):(CUT+referenceCells/2))); 
            gCA = min(LeadingWindow, LaggingWindow);
